...
class User extends Admin_Base_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('percobaan_model');
    ...
    {
        $this->data['title'] = 'Percobaan Login Aplikasi';
        $this->data['breadcrumbs'] = 'Percobaan Login Aplikasi';
        $data['dtlogin'] =  $this->percobaan_model->get_all();
        $this->load->view('admin/user/attempts', $data);
    }
    ...
    public function get_all()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->ion_auth->is_admin()) {
                ...
                $this->data['all'] = $this->user_model->get_all_users_staff();
            }
    ...
    public function deletepercobaans($id)
    {
        $result = $this->percobaan_model->delete($id);
        redirect('admin/user/attempts');
    }
    ...
    public function resetpwd()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            $result = $this->ion_auth->reset_password_user($id);
            
            if ($result) {
               
                $response_array['type'] = 'success';
                $response_array['message'] = '<div class="alert alert-success alert-dismissable"><i class="icon fa fa-check"></i> Successfully Reset Password <br>Password default is : <b>Meyzuser01</b> </div>';
                echo json_encode($response_array);
            } else {
                $response_array['type'] = 'danger';
                $response_array['message'] = '<div class="alert alert-danger alert-dismissable"><i class="icon fa fa-times"></i> Reset Password! Failed.</div>';
                echo json_encode($response_array);
            }
        }
    }
