...
<script type="text/javascript">
 $(document).ready(function () {
     ...
     var headersource =
     {
         ...
         url: '<?=base_url('admin/json/getdt_byrpaket_umroh'); ?>',
         id: 'idpaid',
         async: false
     };
     var dataAdapterHeader = new $.jqx.dataAdapter(headersource);

     var headersourcefinish =
         {
             datatype: "json",
             datafields: [
                 { name: 'idpaid',type: 'int'},
             ...
             url: '<?=base_url('admin/json/getdt_byrpaket_umroh_finish'); ?>',
             id: 'idpaid',
             async: false
         };
     var dataAdapterFinish = new $.jqx.dataAdapter(headersourcefinish);
     ...
     $("#jqxgrid").jqxGrid(
             {
                 ...
                 columns: [
                     ...
                     {
                         text: '', columngroup:'statusbyr', filtertype: 'none', width: 160, cellsalign: 'right',columntype:'button',cellsclass:'btn btn-danger',
                         cellsrenderer: function (row, column, value) {
                         ...
                         if (rowData.status ==1) {
                             bootbox.confirm({
                                 ...
                                 callback: function (result) {
                                     if (result) {
                                         window.location.href = '<?php echo site_url("admin/history/konfirm_bayar");?>/'+rowData.idpaid+'/'+rowData.idorder;
                                     }
                                 }
                             });
                         } else {
                     ...
                     {
                         text: '', columngroup:'statusbyr', filtertype: 'none', width: 145, cellsalign: 'right',columntype:'button',cellsclass:'btn btn-danger',
                         cellsrenderer: function (row, column, value) {
                             var grid = $('#' + this.owner.element.id);
                             var rowData = grid.jqxGrid('getrowdata', row);
                             if (rowData.status ==1) {
                                     ...
                                     callback: function (result) {
                                         if (result) {
                                             bootbox.prompt({
                                                 ...
                                                 centerVertical: true,
                                                 callback: function(result) {
                                                     window.location.href = '<?php echo site_url("admin/history/hapus_bayar");?>/'+rowData.idpaid+'/'+result;
                                                 }
                                             });
                                         }
                                     }
