     ...
     $("#jqxgrid").jqxGrid(
             {
                 ...
                 columns: [
                     ...
                     //     }
                     // },
                     { text: 'Nama Paket',columntype: 'textbox', filtertype: 'textbox',datafield: 'name',align: 'center',cellsalign: 'left'},
                     { text: 'Pilihan Paket',columntype: 'textbox', filtertype: 'list',datafield: 'type',align: 'center',cellsalign: 'left',width: '135'},
                     { text: 'Total Hari',columntype: 'textbox', filtertype: 'textbox',datafield: 'day',align: 'center',cellsalign: 'center',width: '100'},
                     { text: 'Harga Paket',columntype: 'numberinput', filtertype: 'numberinput',cellsformat: 'c0',datafield: 'price',align: 'center',cellsalign: 'right',width: '95'},
                     {
                         text: 'Maskapai',columngroup:'detail', filtertype: 'none', width: 40, cellsalign: 'right',columntype:'button',
                         cellsrenderer: function (row, column, value) {
                             var grid = $('#' + this.owner.element.id);
                             var rowData = grid.jqxGrid('getrowdata', row);
                             var id=rowData.idpackage;
                             return '<span class="fa fa-plane" dtid="' + rowData.idpackage + '" onclick="load_modal_airlines('+id+')" data-toggle="tooltip" title="Silahkan tekan icon untuk melihat detail."></span>';
                         }
                     },
                     {
                         text: 'Hotel',columngroup:'detail', filtertype: 'none', width: 40, cellsalign: 'right',columntype:'button',
                         cellsrenderer: function (row, column, value) {
                             var grid = $('#' + this.owner.element.id);
                             var rowData = grid.jqxGrid('getrowdata', row);
                             var id=rowData.idpackage;
                             return '<span  class="fa fa-bed" onclick="load_modal_hotels('+id+')" data-toggle="tooltip" title="Silahkan tekan icon untuk melihat detail."></span>';
                         }
                     },
                     {
                         text: 'Bus',columngroup:'detail', filtertype: 'none', width: 40, cellsalign: 'right',columntype:'button',
                         cellsrenderer: function (row, column, value) {
                             var grid = $('#' + this.owner.element.id);
                             var rowData = grid.jqxGrid('getrowdata', row);
                             var id=rowData.idpackage;
                             return '<span  class="fa fa-bus" onclick="load_modal_bus('+id+')" data-toggle="tooltip" title="Silahkan tekan icon untuk melihat detail."></span>';
                         }
                     },
                     {
                         text: 'Item Include', columngroup:'item',filtertype: 'none', width: 65, cellsalign: 'right',columntype:'button',cellsclass:'btn btn-info',
                         cellsrenderer: function (row, column, value) {
                             var grid = $('#' + this.owner.element.id);
                             var rowData = grid.jqxGrid('getrowdata', row);
                             return '<span  class="fa fa-list-ol" onclick="load_modal_include('+rowData.idpackage+')" data-toggle="tooltip" title="Silahkan tekan icon untuk melihat detail."></span>';

                         }
                     },
                     {
                         text: 'Item Free', columngroup:'item',filtertype: 'none', width: 65, cellsalign: 'right',columntype:'button',cellsclass:'btn btn-info',
                         cellsrenderer: function (row, column, value) {
                             var grid = $('#' + this.owner.element.id);
                             var rowData = grid.jqxGrid('getrowdata', row);
                             return '<span  class="fa fa-list-ol" onclick="load_modal_free('+rowData.idpackage+')" data-toggle="tooltip" title="Silahkan tekan icon untuk melihat detail."></span>';
                         }
                     },
