...
class Ion_auth
{
    ...
    public function is_staff_bst($id = FALSE)
    {
        $this->ion_auth_model->trigger_events('is_admin_bst');

        $staff_bst_group = $this->config->item('staff_bst_group', 'ion_auth');

        return $this->ion_auth_model->in_group($staff_bst_group, $id);
    }

    public function is_agent($id = FALSE)
    {
        $this->ion_auth_model->trigger_events('is_agent');

        $agent_group = $this->config->item('agent_group', 'ion_auth');

        return $this->ion_auth_model->in_group($agent_group, $id);
    }

    public function is_ceo($id = FALSE)
    {
        $this->ion_auth_model->trigger_events('is_ceo');

        $ceo_group = $this->config->item('ceo_group', 'ion_auth');

        return $this->ion_auth_model->in_group($ceo_group, $id);
    }
